...
**Обоснование отдельного файла:** context.go группирует 3 функции единой подсистемы. Не раздувает metrics.go (уже ~300 LOC). Аналогично similarity.go.
...
В каждой итерации execute/review:
```go
...
r.Metrics.RecordSession(sessionMetrics, stepType, durationMs, compactions)
```
...
Принимает `*session.SessionMetrics` целиком (не 5 отдельных int'ов) — расширяемо, DRY. `contextWindow` теперь доступен как `metrics.ContextWindow` (из `modelUsage` в JSON). Caller применяет fallback: `if metrics.ContextWindow == 0 { contextWindow = 200000 }`.
...
Текущая сигнатура (из overview):
```go
func (mc *MetricsCollector) RecordSession(metrics *session.SessionMetrics, model, stepType string, durationMs int64)
```

Новая:
```go
func (mc *MetricsCollector) RecordSession(metrics *session.SessionMetrics, model, stepType string, durationMs int64, compactions int, contextFillPct float64)
```
...
**Альтернатива:** передавать `compactions` и `contextFillPct` как отдельный вызов (e.g. `RecordContextMetrics`). Отвергнута: эти данные привязаны к конкретной сессии, логично передавать вместе с session metrics.
...
|---------|---------|-----------|
| fill ≤ warn threshold | — (тихо) | Только в session log |
| fill > warn, ≤ critical | WARN | `"context fill NN.N% — consider reducing max_turns (current: N) or splitting task"` |
| fill > critical | ERROR | `"context fill NN.N% exceeds critical threshold — reduce max_turns (current: N)"` |
| compactions > 0 | ERROR | `"N compaction(s) detected — context was compressed, reduce max_turns (current: N)"` |
...
При `compactions > 0` или `fill > criticalPct` — строка окрашивается (жёлтый/красный через `fatih/color`).
...
Это **breaking change** для пользователей с `max_turns: 50` по дефолту. Митигация: пользователь может вернуть через config или `--max-turns` CLI.
...
| RecordSession signature change ломает callers | Все callers в runner.go — единственный файл. Обновить все call sites одновременно |
| max_turns:15 — breaking default | Документировать в CHANGELOG. CLI --max-turns override |
