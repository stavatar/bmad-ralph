...
### Дополнения из ретроспективы Эпика 1

**Источник:** Исследование `docs/research/knowledge-extraction-in-claude-code-agents.md` (20 источников, Anthropic, GitHub, Chroma Research).

Ретроспектива Эпика 1 выявила 4 архитектурных пробела. Ниже — конкретные дополнения к историям:

**Пробел 1: Валидация ссылок на источник (истории 6.1, 6.2, 6.6, 6.7)**
Каждая запись в LEARNINGS.md должна содержать ссылку на источник: `[История X.Y, файл.go:строка]`. При дистилляции (6.2) — проверять ссылки, помечать устаревшие для удаления. Обоснование: система памяти GitHub Copilot с валидацией по ссылкам даёт +7% к слиянию запросов.

**Пробел 2: Атомарный формат записей (истории 6.1, 6.2, 6.6, 6.7)**
Записи должны быть независимыми фактами с тематическими заголовками, а не связным повествованием. Исследование деградации контекста (Chroma, 18 моделей) показывает: разрозненные факты эффективнее организованного текста на 30-50%.

Формат записи:
```
## тестирование: нулевые значения [История 1.2, config/errors.go:42]
ExitCodeError{} и GateDecision{} — тест нулевых значений ловит неинициализированные поля.
```

**Пробел 3: Внедрение через хуки жизненного цикла (история 6.5)**
Оценить использование хука SessionStart для внедрения критических правил из LEARNINGS.md. Обходит проблему оформления CLAUDE.md («может быть релевантным, а может нет»). Стоимость: ~750 токенов на 50 ходов. Требуется проверка: работают ли хуки при вызове `claude --print`/`--resume`.

**Пробел 4: Метрики петли обратной связи (история 6.5)**
Добавить в логирование: количество строк в LEARNINGS.md, количество находок рецензии. Если оба показателя растут одновременно — обнаружена петля обратной связи, требуется агрессивная очистка.
