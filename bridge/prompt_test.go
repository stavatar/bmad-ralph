...
func TestBridgePrompt_Creation(t *testing.T) {
	...
	checks := []struct {
		...
		{"SETUP ordering rule", "BEFORE any implementation tasks", true},
		{"VERIFY ordering rule", "after related implementation", true},
		{"source multiple ACs rule", "PRIMARY AC number", true},
		{"source identifier SETUP", "#SETUP", true},
		{"source identifier VERIFY", "#VERIFY", true},
