...
- Each task = one Claude Code session. Sessions are isolated — each runs without memory of previous tasks, so every task must be self-contained. Minimize task count while maintaining full AC coverage.
...
### WRONG (over-packed / too complex):

A single task should NOT combine multiple independent concerns. Split when:
- **Refactoring + new feature**: replacing an existing component AND adding new behavior are separate tasks. E.g., "Replace overlay with WizardSection" is task 1, "Add skip button to hook" is task 2.
- **Alternatives in description**: if a task says "X or Y" — pick ONE approach and make it the task. Do NOT give Claude a choice inside a task description.
- **Multiple independent UI/API changes**: "Add endpoint A and refactor component B and update tests for C" — split by independent change unit.
- **5+ ACs spanning different files/components with independent concerns**: when ACs touch different layers (hook + page + component + API), split by layer boundary. Note: 5+ ACs all modifying one file (e.g., adding 6 validators to one DTO) is still ONE task.

### Complexity ceiling:

Each task should be completable in 1-3 review cycles. Heuristics for "too complex":
- Task description contains "or" offering alternative approaches → split into the chosen approach only
- Task touches 4+ unrelated files across different concerns → split by concern
- Task combines refactoring existing code with writing new code → split: refactor first, then add
- Task description exceeds ~300 characters → likely over-packed, consider splitting (weakest signal — only split if semantic criteria above also indicate it; long descriptions due to listing test scenarios are fine)

### Signals to keep as ONE task (do not split):

- All changes are in the same file or file+test pair
- Removing either "half" would break compilation or tests — the parts are not independently verifiable
- The split would produce a task with no independently testable outcome
- Multiple Behavioral ACs that test the same Implementation AC
- Story has 1-2 simple ACs → always 1 task, do not pad the output with extra tasks

### Task ordering:

If task B depends on task A (e.g., B uses a database table created by A's migration, or B imports a module created by A), task A MUST appear before task B in the output.

### Correct (split complex story):

Story BF-2.1 has 5 ACs: remove overlay, add skip, verify accordion, verify skip works, verify sections expand → **2-3 tasks:**
1. `- [ ] Replace inline wizard overlay in AiSettingsPage with WizardSection component, with test verifying no overlay when wizard completed [GATE]`
2. `- [ ] Add skip button handling: verify WizardSection onSkip closes wizard and page remains functional, with test for skip and accordion accessibility`

NOT 1 mega-task: `- [ ] Replace overlay (or add skip button), with tests for overlay, skip, and accordion` ← too many concerns, alternative approaches, Claude will thrash
...
- Security-critical changes requiring human sign-off (authentication, encryption, access control).

Look for keywords like "deploy", "release", "production", "staging", "launch", "go-live", "security", "encryption" in story content to identify milestones. Do NOT mark routine feature tasks as milestones — gates should be rare.
...
- Modified tasks: if a `- [ ]` (open) task description changed, update the description text.
- NEW work on completed tasks: if an `[x]` task needs additional work from an updated story, create a NEW `- [ ]` task referencing the same ACs. Do NOT modify the completed task's description — it represents work that was already done.
